...
by Carapace. Commands, flags, and fixed values are generated statically.
Completing thread and project references runs a small helper that calls Amp,
so those completions reflect the resources available to the signed-in user.
...
Python 3 and may make an Amp API request when completing a thread or project.
...
and visibility levels. For account-specific values, the generated spec invokes
`amp-completions`, which converts `amp threads list --json` and
`amp projects list --json` into Carapace candidates.
...
thread and project completion runs Amp.
